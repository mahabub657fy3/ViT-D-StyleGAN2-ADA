...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
        ...
        scale=0.6, rotate=0, aniso=0, xfrac=0, scale_std=0.4, rotate_max=1, aniso_std=0.2, xfrac_std=0.125,
        brightness=0, contrast=0.3, lumaflip=0, hue=1, saturation=0.7, brightness_std=0.3, contrast_std=0.5, hue_max=1, saturation_std=1,
        imgfilter=0, imgfilter_bands=[1,1,1,1], imgfilter_std=1,
        noise=0, cutout=0, noise_std=0.1, cutout_size=0.5,patch_dropout=0.1, patch_shuffle=0.1, 
        ...
        self.patch_shuffle = float(patch_shuffle) 
    ...
    def forward(self, images): 
        ...
        images = self.apply_standard_augmentations(images)

        # Apply ViT-specific augmentations.
        images = self.apply_vit_specific_augmentations(images)

        return images

    def apply_standard_augmentations(self, images,debug_percentile=None):
    ...
    def apply_vit_specific_augmentations(self, images, patch_size=16):
   
        b, c, h, w = images.shape  # Batch size, channels, height, width

        # Ensure the image dimensions are divisible by the patch size
        if h % patch_size != 0 or w % patch_size != 0:
            raise ValueError(f"Image dimensions ({h}, {w}) must be divisible by the patch size ({patch_size}).")

        # Apply Color Jitter
        images = self.apply_color_jitter(images)

    # Apply Patch Dropout
        if self.patch_dropout > 0:
            patch_h, patch_w = h // patch_size, w // patch_size  
            mask = torch.rand((b, patch_h, patch_w), device=images.device) < self.patch_dropout
            mask = mask.repeat_interleave(patch_size, dim=1).repeat_interleave(patch_size, dim=2)  
            images = images * (~mask).unsqueeze(1) 

        # Apply Patch Shuffling
        if self.patch_shuffle > 0:
            patch_h, patch_w = h // patch_size, w // patch_size
            reshaped = images.view(
                b, c, patch_h, patch_size, patch_w, patch_size
            )  # Reshape to separate patches and their spatial dimensions
            patches = reshaped.permute(0, 2, 4, 1, 3, 5).reshape(
                b, patch_h * patch_w, c * patch_size * patch_size
            )  # Flatten patches and reorder dimensions
            perm = torch.randperm(patch_h * patch_w)  # Generate a random permutation
            shuffled = patches[:, perm].reshape(
                b, patch_h, patch_w, c, patch_size, patch_size
            )  # Apply the permutation and reshape back
            images = shuffled.permute(0, 3, 1, 4, 2, 5).reshape(
                b, c, h, w
            )  # Rearrange and reshape to the original image dimensions

        # Apply Affine Transformations
        images = self.apply_affine_transformations(images)
    ...
    def apply_color_jitter(self, images):
        ...
        if self.brightness > 0:
            brightness_factor = torch.randn((images.size(0), 1, 1, 1), device=images.device) * self.brightness_std
            images = images + brightness_factor.clamp(-self.brightness, self.brightness)

        if self.contrast > 0:
            contrast_factor = torch.exp2(torch.randn((images.size(0), 1, 1, 1), device=images.device) * self.contrast_std)
            images = (images - 0.5) * contrast_factor + 0.5

        if self.hue > 0:
            hue_factor = (torch.randn((images.size(0), 1, 1, 1), device=images.device) * self.hue_max).clamp(-self.hue_max, self.hue_max)
            images = torch.cos(hue_factor) * images - torch.sin(hue_factor) * images

        if self.saturation > 0:
            saturation_factor = torch.exp2(torch.randn((images.size(0), 1, 1, 1), device=images.device) * self.saturation_std)
            images = images * saturation_factor

        return images.clamp(0, 1)

    def apply_affine_transformations(self, images):
    
        b, c, h, w = images.shape
        theta = torch.eye(2, 3).unsqueeze(0).repeat(b, 1, 1).to(images.device)

        # Random rotations
        if self.rotate > 0:
            angles = (torch.rand(b, device=images.device) - 0.5) * 2 * np.pi * self.rotate
        ...
        # Random scaling
        if self.scale > 0:
            scales = 1 + (torch.rand(b, device=images.device) - 0.5) * 2 * self.scale_std
            theta[:, 0, 0] *= scales
            theta[:, 1, 1] *= scales

        # Random translation
        if self.xfrac > 0:
            translations = (torch.rand(b, 2, device=images.device) - 0.5) * self.xfrac_std
